    ...
    internal class Evade
    {
        ...
        private void Game_OnGameLoad()
        {
            try
            {
                ...
                mainMenu.Add(new MenuBool("DodgeDangerous", "只躲避危险技能", false));
                mainMenu.Add(new MenuBool("DodgeCircularSpells", "躲避圆形技能"));
                mainMenu.Add(new MenuKeyBind("ActivateEvadeSpells", "使用技能躲避", KeyCode.K,
                    KeybindType.Toggle));
                mainMenu.Add(new MenuBool("DodgeFOWSpells", "躲避战争迷雾技能"));
                menu.Add(mainMenu);

                keyMenu = new Menu("KeyMenu", "按键设置");
                keyMenu.Add(new MenuBool("DodgeOnlyOnComboKeyEnabled", "躲避只在按键按下啟用", false));
                keyMenu.Add(new MenuKeyBind("DodgeComboKey", "躲避按键", KeyCode.Space, KeybindType.Press));
                keyMenu.Add(new MenuBool("DodgeDangerousKeyEnabled", "躲避危险按鍵啟用", false));
                keyMenu.Add(new MenuKeyBind("DodgeDangerousKey", "只躲避危险技能 按键", KeyCode.Space,
                    KeybindType.Press));
                keyMenu.Add(new MenuKeyBind("DodgeDangerousKey2", "只躲避危险技能 按键2", KeyCode.V,
                    KeybindType.Press));
                keyMenu.Add(new MenuBool("DontDodgeKeyEnabled", "啟用不躲避按鍵", false));
                keyMenu.Add(new MenuKeyBind("DontDodgeKey", "不躲避按鍵", KeyCode.Z, KeybindType.Press));
                menu.Add(keyMenu);

                var loadTestMenu = new Menu("LoadTests", "测试")
                {
                    new MenuBool("LoadPingTester", "读取游戏延时文本", false),
                    new MenuBool("LoadSpellTester", "读取加载文本", false)
                };
                ...
                miscMenu.Add(new MenuBool("ClickRemove", "单击技能线圈取消躲避"));
                miscMenu.Add(new MenuBool("CalculateWindupDelay", "计算终结延时"));
                miscMenu.Add(new MenuBool("AdvancedSpellDetection", "高级法术检测", true));
                miscMenu.Add(new MenuBool("CheckSpellCollision", "检测技能碰撞"));
                miscMenu.Add(new MenuList("EvadeMode", "躲避模式",
                    new[] {"平滑躲避", "缓慢躲避", "快速躲避", "Hawk", "Kurisu", "GuessWho"}, 0));
                miscMenu.Add(new MenuBool("PreventDodgingUnderTower", "塔下躲避"));
                miscMenu.Add(new MenuBool("PreventDodgingNearEnemy", "躲避防止接近敵人"));
                //miscMenu.Add(new MenuBool("DrawEvadePosition", "畫出躲避位置", false));
                miscMenu.Add(loadTestMenu);
                menu.Add(miscMenu);

                miscMenu["EvadeMode"].OnValueChanged += OnEvadeModeChange;

                bufferMenu = new Menu("BufferMenu", "額外缓冲設置");
                bufferMenu.Add(new MenuSlider("ExtraSpellRadius", "額外技能半径", 0, 0, 100));
                bufferMenu.Add(new MenuSlider("ExtraPingBuffer", "額外ping缓冲", 65, 0, 200));
                bufferMenu.Add(new MenuSlider("ExtraAvoidDistance", "額外的避免距离", 50, 0, 300));
                bufferMenu.Add(new MenuSlider("ExtraEvadeDistance", "額外的回避距离", 100, 0, 300));
                bufferMenu.Add(new MenuSlider("ExtraCPADistance", "額外碰撞距離", 10, 0, 150));
                bufferMenu.Add(new MenuSlider("MinComfortZone", "最小緩和", 550, 0, 1000));

                menu.Add(bufferMenu);

                limiterMenu = new Menu("LimiterMenu", "人性化設置");
                limiterMenu.Add(new MenuSlider("SpellDetectionTime", "法术检测时间", 0, 0, 1000));
                limiterMenu.Add(new MenuSlider("ReactionTime", "躲避反应时间", 0, 0, 500));
                limiterMenu.Add(new MenuSlider("DodgeInterval", "躲避间隔", 0, 0, 2000));
                limiterMenu.Add(new MenuSlider("TickLimiter", "点击限制", 100, 0, 500));
                limiterMenu.Add(new MenuBool("EnableEvadeDistance", "延時躲避"));
                limiterMenu.Add(new MenuBool("ClickOnlyOnce", "只點击一次"));
                menu.Add(limiterMenu);

                fastEvadeMenu = new Menu("FastEvade", "快速躲避菜单");
                fastEvadeMenu.Add(new MenuBool("FastMovementBlock", "快速移动"));
                fastEvadeMenu.Add(new MenuSlider("FastEvadeActivationTime", "快速躲避激活時間", 65, 0, 500));
                fastEvadeMenu.Add(new MenuSlider("SpellActivationTime", "技能激活時間", 400, 0, 1000));
                fastEvadeMenu.Add(new MenuSlider("RejectMinDistance", "碰撞距离缓冲区域", 10, 0, 100));
